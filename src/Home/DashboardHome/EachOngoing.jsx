...
const EachOngoing = ({ task }) => {
    ...
    const handleMove = id => {
        axiosPublic.patch(`/tasks/${id}/done`)
            .then(res => {
                // console.log(res.data);
                if (res.data.modifiedCount > 0) {
                    refetch();
                    swal("Good job!", `Its done now`, "success");
        ...
        <div className="p-2 mb-2 min-h-[90px] mx-2 bg-black cursor-pointer flex flex-col justify-between border border-yellow-500 hover:rounded-xl">
            ...
            <div className="flex flex-col justify-center p-1">
                ...
                <p className="text-red-500 text-center"><small>Deadline: {task?.deadline}</small></p>
                    <button onClick={() => handleMove(task?._id)} className="btn btn-xs"><small className="flex flex-row items-center justify-center gap-1"><span className="uppercase">Move to Done</span> <span><FaArrowAltCircleRight /></span></small></button>
